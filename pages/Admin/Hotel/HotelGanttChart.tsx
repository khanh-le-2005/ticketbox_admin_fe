...
import {
    ChevronDown, Plus, Search, Filter, Settings,
    MoreHorizontal, CheckCircle, Clock, User,
    Edit, LogOut, Trash2, Calendar, Building2,
    ChevronLeft, ChevronRight, SlidersHorizontal, LayoutList, Grid3X3, Brush, Construction,
    BedDouble, ArrowRightLeft
} from 'lucide-react';
import { Listbox, Transition } from '@headlessui/react';
import { format, addDays, parseISO, isToday, differenceInDays, startOfDay, isWeekend, eachDayOfInterval } from 'date-fns';
...
interface ContextMenuPosition { x: number; y: number; roomId?: string; date?: Date; bookingId?: string; }
...
export default function HotelGanttChart() {
    ...
    const [startDate, setStartDate] = useState(startOfDay(new Date('2026-01-28'))); // Mock date theo data của bạn
...
const dates = useMemo(() => {
    ...
    const handleMouseUp = () => {
        if (isDragging && selection) console.log("Selected:", selection);
        setIsDragging(false);
    };
        ...
        <div className="flex flex-col h-screen bg-slate-50 text-sm font-sans text-slate-700 selection:bg-indigo-100">
            ...
            <div className="flex flex-col z-40 bg-white border-b border-slate-200 shadow-sm relative">
                ...
                <div className="h-16 px-6 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-2 text-indigo-600 font-black text-lg tracking-tight">
                            ...
                            <span>PMS<span className="text-slate-400">.booking</span></span>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                         <button className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-md shadow-indigo-200 transition-all active:scale-95">
                ...
                <div className="h-14 px-6 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 shadow-sm transition"><Settings size={16}/></button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-hidden relative flex bg-white" onMouseUp={handleMouseUp}>
                <div className="overflow-auto w-full h-full relative scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                    ...
                    {/* --- HEADER (DATES) --- */}
                    <div className="sticky top-0 z-30 flex bg-white border-b border-slate-200 shadow-sm" style={{ height: HEADER_HEIGHT, minWidth: 'fit-content' }}>
                        {/* Corner Cell */}
                        <div className="sticky left-0 z-40 bg-white border-r border-slate-200 flex flex-col items-center justify-center shadow-[4px_0_12px_-4px_rgba(0,0,0,0.05)]" 
                            ...
                            <div className="mt-2 flex gap-2">
                                <span className="px-2 py-0.5 bg-slate-100 rounded text-[10px] font-bold text-slate-500">{hotels.length} KS</span>
                            </div>
                        </div>
                        ...
                        <div className="flex">
                            {dates.map((date, index) => {
                                ...
                                return (
                                    <div key={index}
                                        className={`flex-shrink-0 border-r border-slate-100 flex flex-col items-center justify-center group relative ${isW ? 'bg-slate-50/50' : ''} ${isT ? 'bg-indigo-50/30' : ''}`}
                                        style={{ width: CELL_WIDTH, height: HEADER_HEIGHT }}
                                    >
                                        {isT && <div className="absolute top-0 inset-x-0 h-1 bg-indigo-500"></div>}
                                        <span className={`text-[10px] font-bold uppercase mb-1 ${isT ? 'text-indigo-600' : 'text-slate-400'}`}>
                                            {format(date, 'EEE', { locale: vi })}
                                        </span>
                                        <div className={`w-9 h-9 flex items-center justify-center rounded-full text-lg font-bold transition-all ${isT ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-700 group-hover:bg-slate-200'}`}>
                                            {format(date, 'dd')}
                                        </div>
                                    </div>
                                );
                    ...
                    <div className="relative min-w-fit pb-20">
                        ...
                        {loading && (
                            <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-50 flex items-center justify-center">
                                <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Đang tải dữ liệu...</span>
                                </div>
                            </div>
                        )}
                        ...
                        {roomGroups.map((group) => (
                            <div key={group.id} className="">
                                {/* Group Header */}
                                <div className="sticky left-0 z-20 bg-slate-50/95 border-b border-r border-slate-200 px-4 py-2 flex items-center justify-between backdrop-blur-sm" style={{ width: SIDEBAR_WIDTH }}>
                                ...
                                {group.rooms.map((room) => (
                                    <div key={room.id} className="flex relative hover:bg-slate-50/50 transition-colors group/row">
                                        {/* Sidebar Room Cell */}
                                        <div className="sticky left-0 z-20 bg-white border-r border-b border-slate-100 px-4 flex items-center justify-between group-hover/row:bg-slate-50 transition-colors shadow-[2px_0_10px_-4px_rgba(0,0,0,0.05)]"
                                            ...
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[10px] text-slate-400 capitalize">{room.status === 'clean' ? 'Sạch' : 'Chưa dọn'}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex relative">
                                            {dates.map((date, index) => {
                                                ...
                                                return (
                                                    <div key={index}
                                                        className={`border-r border-b border-slate-100 flex-shrink-0 transition-colors 
                                                            ${isW ? 'bg-slate-50/40' : ''} 
                                                            ${isT ? 'bg-indigo-50/20' : ''}
                                                            hover:bg-indigo-50/30
                                                        `}
                                                        style={{ width: CELL_WIDTH, height: CELL_HEIGHT }}
                                                        onContextMenu={(e) => handleContextMenu(e, room.id, date)}
                                            ...
                                            {isDragging && selection && selection.roomId === room.id && (
                                                <div className="absolute bg-indigo-500/20 border-2 border-indigo-500 z-10 pointer-events-none rounded-lg" style={getSelectionStyle()}>
                                            ...
                                            {bookings.filter(b => b.roomId === room.id).map(booking => {
                                                ...
                                                return (
                                                    <div key={booking.id}
                                                        ...
                                                        style={style}
                                                        onContextMenu={(e) => { e.stopPropagation(); handleContextMenu(e, undefined, undefined, booking.id); }}
                                                    >
                            ...
                            <div className="flex flex-col">
                                <div className="px-4 py-3 bg-indigo-50 border-b border-indigo-100 mb-1">
                                    ...
                                    <span className="font-bold text-indigo-900 text-sm">
                                        {contextMenu.date && format(contextMenu.date, 'EEEE, dd/MM/yyyy', { locale: vi })}
                                    </span>
                                </div>
                                <button className="w-full text-left px-4 py-2.5 hover:bg-indigo-600 hover:text-white flex items-center gap-3 text-slate-700 font-bold text-sm transition-all group">
