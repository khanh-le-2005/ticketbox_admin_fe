...
import { getAllCompanies } from "@/apis/api_company";
import { Company } from "@/type/company.type";
import { toast } from 'react-toastify';
import { LocationOption, GalleryItem } from "@/type/Show.type";
...
const AddShow: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.name?.trim()) {
      return toast.warn("Vui lòng nhập tên Show diễn.");
    }
    if (!formData.startTime) {
      return toast.warn("Vui lòng chọn thời gian bắt đầu.");
    }
    if (!formData.endTime) {
      return toast.warn("Vui lòng chọn thời gian kết thúc.");
    }
    if (!formData.companyId) {
      return toast.warn("Vui lòng chọn Công ty/Đơn vị tổ chức.");
    }

    // Kiểm tra Banner
    if (!bannerPreview) {
      return toast.warn("Vui lòng tải ảnh Banner (Ảnh bìa) cho Show.");
    }

    // Kiểm tra địa chỉ
    if (!formData.address.province) {
      return toast.warn("Vui lòng chọn Tỉnh/Thành phố.");
    }
    if (!formData.address.district) {
      return toast.warn("Vui lòng nhập/chọn Quận/Huyện.");
    }
    if (!formData.address.ward) {
      return toast.warn("Vui lòng nhập/chọn Phường/Xã.");
    }
    if (!formData.address.specificAddress?.trim()) {
      return toast.warn("Vui lòng nhập địa chỉ cụ thể (Số nhà, tên đường...).");
    }

    // Kiểm tra Nghệ sĩ (Thanh lọc performers rỗng)
    const validPerformers = formData.performers.filter(p => p && p.trim() !== "");
    if (validPerformers.length === 0) {
      return toast.warn("Vui lòng nhập ít nhất một nghệ sĩ biểu diễn.");
    }

    // Kiểm tra Hạng vé
    if (!formData.ticketTypes || formData.ticketTypes.length === 0) {
      return toast.warn("Vui lòng thêm ít nhất một hạng vé.");
    }

    for (let i = 0; i < formData.ticketTypes.length; i++) {
      const t = formData.ticketTypes[i];
      if (!t.name?.trim()) {
        return toast.warn(`Hạng vé thứ ${i + 1} chưa có tên.`);
      }
      if (t.price <= 0) {
        return toast.warn(`Giá vé của hạng "${t.name}" phải lớn hơn 0.`);
      }
      if (t.totalQuantity <= 0) {
        return toast.warn(`Số lượng vé của hạng "${t.name}" phải lớn hơn 0.`);
      }
    }
